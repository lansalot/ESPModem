...
static esp_err_t hotspot_auth(httpd_req_t *req) {
    ...
    wifi_sta_mac_ip_list_t ap_sta_ip_list;
    esp_wifi_ap_get_sta_list_with_ip(ap_sta_list, &ap_sta_ip_list);

    // TODO: Correctly read IPv4?
    for (int i = 0; i < ap_sta_ip_list.num; i++) {
        if (ap_sta_ip_list.sta[i].ip.addr == client_addr.sin6_addr.un.u32_addr[3]) return ESP_OK;
    }
